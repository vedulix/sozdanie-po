...
namespace WebAppCoreProduct.Pages
{
    public class ProductModel : PageModel
    {
        ...
        // Упражнение 5: свой обработчик POST — двойная скидка
        public void OnPostDouble(string name, decimal? price)
        {
            Product = new Product();
            if (price == null || price < 0 || string.IsNullOrEmpty(name))
            {
                MessageRezult = "Переданы некорректные данные";
                return;
            }

            var result = price * (decimal?)0.36;
            MessageRezult = $"Для товара {name} с ценой {price} двойная скидка (36%) получится {result}";
            Product.Price = price;
            Product.Name = name;
        }
    }
